...
function getOrigin(req: Request): string {
  // Prefer REPLIT_DOMAINS so the callback URL is always the correct public
  // domain, regardless of how the request was proxied internally.
...
async function upsertUser(claims: Record<string, unknown>) {
  ...
  const isOwner = id === process.env.REPL_OWNER_ID;
  ...
  const [user] = await db
    .insert(usersTable)
    .values({ ...userData, isAdmin: isOwner })
    .onConflictDoUpdate({
